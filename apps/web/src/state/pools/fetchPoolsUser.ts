...
// Pool 0, Base / Base is a different kind of contract (master chef)
// BASE pools use the native BASE token (wrapping ? unwrapping is done at the contract level)
const nonBnbPools = poolsConfig.filter((pool) => pool.stakingToken.symbol !== 'BASE')
const bnbPools = poolsConfig.filter((pool) => pool.stakingToken.symbol === 'BASE')
