...
export function useGasPrice(chainIdOverride?: number): string {
  ...
  const { data } = useFeeData({
    chainId,
    enabled: chainId !== ChainId.BASE && chainId !== ChainId.BSC_TESTNET,
    watch: true,
  })
...
export function toV2LiquidityToken([tokenA, tokenB]: [ERC20Token, ERC20Token]): ERC20Token {
  return new ERC20Token(tokenA.chainId, Pair.getAddress(tokenA, tokenB), 18, 'OnePieceSwap-LP', 'OnePieceSwap LPs')
}
...
export function useTrackedTokenPairs(): [ERC20Token, ERC20Token][] {
  ...
  console.log("useOfficialsAndUserAddedTokens:", tokens)
  // pinned pairs
