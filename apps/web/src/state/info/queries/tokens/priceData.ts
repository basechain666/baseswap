...
/**
 * Price data for token and base based on block number
 */
...
const fetchTokenPriceData = async (
  ...
  try {
    ...
    // format token BASE price results
    const tokenPrices: {
      timestamp: string
      derivedBNB: number
      priceUSD: number
    ...
    // Get Token prices in BASE
    Object.keys(prices).forEach((priceKey) => {
      const timestamp = priceKey.split('t')[1]
      // if its BASE price e.g. `b123` split('t')[1] will be undefined and skip BASE price entry
      if (timestamp) {
        tokenPrices.push({
          timestamp,
    ...
    // Go through BASE USD prices and calculate Token price based on it
    Object.keys(prices).forEach((priceKey) => {
