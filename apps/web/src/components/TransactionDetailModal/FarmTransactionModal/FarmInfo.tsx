...
const FarmResult: React.FC<React.PropsWithChildren<FarmInfoProps>> = ({ pickedData }) => {
  ...
  const { targetRef, tooltip, tooltipVisible } = useTooltip(
    <Flex flexDirection="column">
      <ListStyle>{t('You have received 0.0005 BASE as a first-time BASE Chain user')}</ListStyle>
      <ListStyle>
        {t('You can swap more BASE on')}
        <LinkStyle href="/swap">{t('Swap.')}</LinkStyle>
      </ListStyle>
      <ListStyle>
        {t('Explore more features like')}
        <LinkStyle href="/pools?chainId=1771">{t('Pools')}</LinkStyle>
        {t('and')}
        <LinkStyle href="/prediction?chainId=1771">{t('Win')}</LinkStyle>
        {t('with your ONEPIECE earned.')}
      </ListStyle>
    </Flex>,
    { placement: 'top' },
  )
    ...
    <Box mb="24px">
      {isFirstStepError ? (
        ...
        <Box>
          ...
          <Text as="span">{t('did not succeed on the BASE Chain side. Please copy the')}</Text>
          <Text bold as="span" m="0 4px">
            {t('Transaction ID')}
          </Text>
