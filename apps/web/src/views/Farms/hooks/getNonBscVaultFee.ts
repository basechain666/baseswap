...
export const getNonBscVaultContractFee = async ({
  ...
  try {
    ...
    const exchangeRate = new BigNumber(ORACLE_PRECISION).div(oraclePrice).times(ORACLE_PRECISION) // invert into BASE/ETH price
    ...
    const [encodeMessage, hasFirstTime, estimateGaslimit] = await Promise.all([
      ...
      crossFarmingAddress.estimateGaslimit(Chains.BASE, userAddress, messageType),
    ])
