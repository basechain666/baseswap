...
const StartPage: React.FC<React.PropsWithChildren<StartPageProps>> = ({ goToApprove, goToChange, goToRemove }) => {
  ...
  /**
   * Check if the wallet has the required ONEPIECE allowance to change their profile pic or reactivate
      ...
      <Flex alignItems="center" style={{ height: '48px' }} justifyContent="center">
        <Text as="p" color="failure">
          {!isProfileCostsLoading &&
            ...
            t('%minimum% ONEPIECE required to change profile pic', { minimum: formatBigNumber(minimumCakeRequired) })}
        </Text>
      </Flex>
      {profile.isActive ? (
        <>
          <Message variant="warning" mb="16px">
            <MessageText>
              {t(
                "Before editing your profile, please make sure you've claimed all the unspent ONEPIECE from previous IFOs!",
              )}
            </MessageText>
          </Message>
          <Button
