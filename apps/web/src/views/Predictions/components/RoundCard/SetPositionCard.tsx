...
import { useGetBASEBalance, useGetCakeBalance } from 'hooks/useTokenBalance'
...
const TOKEN_BALANCE_CONFIG = {
  BASE: useGetBASEBalance,
  ONEPIECE: useGetCakeBalance,
}
...
const SetPositionCard: React.FC<React.PropsWithChildren<SetPositionCardProps>> = ({
  ...
  const { isVaultApproved, setLastUpdated } = useCakeApprovalStatus(token.symbol === 'ONEPIECE' ? predictionsAddress : null)
  ...
  // BASE prediction doesn't need approval
  const doesCakeApprovePrediction = token.symbol === 'BASE' || isVaultApproved
  ...
  const handleEnterPosition = async () => {
    ...
    const args = token.symbol === 'ONEPIECE' ? [epoch, valueAsBn.toString()] : [epoch]
