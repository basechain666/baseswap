...
import { Flex, Grid, Box, Text, Button, BASEIcon, ErrorIcon, useTooltip, Skeleton } from '@pancakeswap/uikit'
...
const SetPriceStage: React.FC<React.PropsWithChildren<SetPriceStageProps>> = ({
  ...
  const { tooltip, tooltipVisible, targetRef } = useTooltip(
    <>
      <Text>
        {t(
          'When selling NFTs from this collection, a portion of the BASE paid will be diverted before reaching the seller:',
        )}
      </Text>
      {creatorFeeAsNumber > 0 && (
        <Text>{t('%percentage%% royalties to the collection owner', { percentage: creatorFee })}</Text>
      )}
      <Text>{t('%percentage%% trading fee will be used to buy & burn ONEPIECE', { percentage: tradingFee })}</Text>
    </>,
    { placement: 'auto' },
  )
    ...
    <>
      ...
      <GreyedOutContainer>
        ...
        <Flex>
          <Flex flex="1" alignItems="center">
            <BASEIcon width={24} height={24} mr="4px" />
            <Text bold>WETH</Text>
          </Flex>
          <Flex flex="2">
            <RightAlignedInput
      ...
      <Grid gridTemplateColumns="32px 1fr" p="16px" maxWidth="360px">
        ...
        <Box>
          ...
          <Text small color="textSubtle">
            {t('Sales are in WETH. You can swap WETH to BASE 1:1 for free with OnePieceSwap.')}
          </Text>
        </Box>
      </Grid>
      <Divider />
