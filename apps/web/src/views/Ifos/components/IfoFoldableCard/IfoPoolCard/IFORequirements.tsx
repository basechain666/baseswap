...
const NotOkNFT = ({ admissionProfile }) => {
  ...
  const keyword = '%Base Squad NFT%'

  const rawText = t(`Set %Base Squad NFT% as Base Profile avatar`)
      ...
      element={
        <>
          <LinkExternal
            ...
            href={`https://BaseSwap.finance/nfts/collections/${admissionProfile}`}
          >
            {t('Base Squad NFT')}
          </LinkExternal>
          <br />
...
const NotOkProfilePoints = ({ pointThreshold }) => {
  ...
  const keyword = '%Base Profile%'

  const rawText = t(`Reach %point% or more %Base Profile% points`, { point: pointThreshold })
    ...
    <TransWithElement
      ...
      element={
        <>
          ...
          <LinkExternal
            style={{ display: 'inline' }}
            href={`https://BaseSwap.finance/profile/${account}/achievements/`}
          >
            {t('Base Profile')}
          </LinkExternal>
        </>
