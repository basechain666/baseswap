...
const CommitTokenSection: React.FC<React.PropsWithChildren<TokenSectionProps & { commitToken: Token }>> = ({
  ...
  if (commitToken.equals(cakeBnbLpToken)) {
    return <TokenSection primaryToken={baseTokens.cake} secondaryToken={baseTokens.weth} {...props} />
  }
...
const IfoCardTokens: React.FC<React.PropsWithChildren<IfoCardTokensProps>> = ({
  ...
  const { targetRef, tooltip, tooltipVisible } = useTooltip(
    t(
      'Sorry, you didn’t contribute enough ONEPIECE to meet the minimum threshold. You didn’t buy anything in this sale, but you can still reclaim your ONEPIECE.',
    ),
    { placement: 'bottom' },
  )
          ...
          <Box>
            <MessageText display="inline">
              {publicIfoData.status === 'finished'
                ? t('Activate OnePieceSwap Profile to take part in next IFO‘s!')
                : t('You need an active OnePieceSwap Profile to take part in an IFO!')}
            </MessageText>{' '}
          ...
          {ifov31Msg || (
            <>
              <Text textAlign="center" fontSize="14px">
                {t('To participate in the next IFO, lock some ONEPIECE in the fixed-term staking ONEPIECE pool!')}
              </Text>
