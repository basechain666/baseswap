...
export const getEarningsText = (
  ...
  if (numFarmsToCollect > 0 && hasCakePoolToCollect) {
    if (numFarmsToCollect > 1) {
      earningsText = t('%earningsBusd% to collect from %count% farms and ONEPIECE pool', data)
    } else {
      earningsText = t('%earningsBusd% to collect from %count% farm and ONEPIECE pool', data)
    }
    ...
    earningsText = t('%earningsBusd% to collect from ONEPIECE pool', data)
  }
