...
import { ONEPIECE } from '@pancakeswap/tokens'
...
const DepositAction: React.FC<React.PropsWithChildren<DepositActionProps>> = ({ totalValueLockedValue }) => {
  ...
  const { balance: userCake } = useTokenBalance(ONEPIECE[chainId]?.address)
  ...
  const { targetRef, tooltip, tooltipVisible } = useTooltip(
    t(
      'ONEPIECE deposit will be diverted to the fixed-term staking pool. Please note that ONEPIECE deposited can ONLY be withdrawn after 10 weeks.',
    ),
    {
      placement: 'bottom',
    },
      ...
      <InputPanel>
        ...
        {isLessThanOneCake && (
          <Text color="failure" fontSize="14px" textAlign="right">
            {t('Please deposit at least 1 ONEPIECE to participate in the Pottery')}
          </Text>
        )}
      </InputPanel>
      <Flex>
        <Flex ml="auto">
          <Text fontSize="12px" color="textSubtle">
            {t('Deposited ONEPIECE will be locked for 10 weeks')}
          </Text>
          <Flex ref={targetRef}>
