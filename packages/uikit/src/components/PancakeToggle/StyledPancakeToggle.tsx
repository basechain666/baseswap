...
const scaleKeyValues = {
  sm: {
    pancakeSize: "14px", // The size of a base (the handle)
    ...
    pancakeThickness: "1px", // Bottom shadow of a base
    pancakeTwoOffset: "0px", // Pancakes don't look good when they are concentric
    pancakeThreeOffset: "-3px", // so base 2 and 3 are shifted a little bit
...
export const PancakeStack = styled.div<HandleProps>`
  ...
  .base:nth-child(3):before,
  .base:nth-child(3):after {
    content: "";
    position: absolute;
    background: #ef8927;
    border-radius: 20px;
...
export const PancakeInput = styled.input<InputProps>`
  ...
  &:checked + label .base:nth-child(1) {
    background: #e27c31;
    box-shadow: 0 ${getScale("pancakeThickness")} 0 ${getScale("pancakeThickness")} #fbbe7c;
    transition-delay: 0.2s;
  }

  &:checked + label .base:nth-child(2) {
    transform: scale(1);
    transition-delay: 0.2s;
  }

  &:checked + label .base:nth-child(3) {
    transform: scale(1);
