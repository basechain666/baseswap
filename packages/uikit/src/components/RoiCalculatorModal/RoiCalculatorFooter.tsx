...
const RoiCalculatorFooter: React.FC<React.PropsWithChildren<RoiCalculatorFooterProps>> = ({
  ...
  const {
    ...
    <>
      <Text>
        {t(
          "The Multiplier represents the proportion of ONEPIECE rewards each farm receives, as a proportion of the ONEPIECE produced each block."
        )}
      </Text>
      <Text my="24px">
        {t("For example, if a 1x farm received 1 ONEPIECE per block, a 40x farm would receive 40 ONEPIECE per block.")}
      </Text>
          ...
          <Grid gridTemplateColumns="2.5fr 1fr" gridRowGap="8px" gridTemplateRows={`repeat(${gridRowCount}, auto)`}>
            ...
            {isFarm && (
              <>
                ...
                <Text color="textSubtle" small>
                  *{t("Base APR (ONEPIECE yield only)")}
                </Text>
                <Text small textAlign="right">
                  {apr.toFixed(2)}%
                </Text>
