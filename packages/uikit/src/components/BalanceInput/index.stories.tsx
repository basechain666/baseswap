...
export const SwitchUnits: React.FC<React.PropsWithChildren> = () => {
  const CAKE_PRICE = 69;
  const [editingUnit, setEditingUnit] = useState<"ONEPIECE" | "USD">("ONEPIECE");
  const conversionUnit = editingUnit === "ONEPIECE" ? "USD" : "ONEPIECE";
  const [values, setValues] = useState({
    ONEPIECE: "1006.086957",
  ...
  const switchEditingUnits = () => {
    const editingUnitAfterChange = editingUnit === "ONEPIECE" ? "USD" : "ONEPIECE";
  ...
  const handleCakeChange = (input: string) => {
    ...
    if (editingUnit === "ONEPIECE") {
      ...
      setValues({
        ONEPIECE: Number.isNaN(inputAsFloat) ? "" : `${inputAsFloat / CAKE_PRICE}`,
        USD: input,
      });
    }
  };
